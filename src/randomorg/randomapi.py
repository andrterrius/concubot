...
async def compose_api_call(json_method_name, *args, **kwargs):
    """
...
async def http_request(url, json_string):
    """
    ...
    async with ClientSession() as session:
        response = await session.post(url, json=json_string, headers={"Content-Type": "application/json"})
        response_string = await response.json()
...
#     response.close()
#
#     return asyncio.run(async_http_request(url, json_string))
...
class RandomJSONRPC:
    ...
    async def delay_request(self, requested_delay):
    ...
    async def send_request(self, request_string, method=""):
        ...
        if self._advisory_delay > 0:
            await self.delay_request(self._advisory_delay)

        # Make the connection now
        json_string = await http_request(JSON_URL, request_string)
        self._time_of_last_request = time.time()
...
####################### RANDOM.org API methods ##########################
    ...
    async def generate_integers(self, n, min, max, replacement=True, base=10):
        '''Returns a list of true random integers with a user-defined range'''
        request_string = await compose_api_call(
            INTEGER_METHOD, apiKey=self.api_key,
            n=n, min=min, max=max, replacement=replacement, base=base)
        return await self.send_request(request_string, INTEGER_METHOD)

    async def generate_integer_sequences(self, n, length, min, max, replacement=True, base=10):
        ...
        return await self.send_request(request_string, INTEGER_SEQUENCE_METHOD)

    async def generate_decimal_fractions(self, n, decimal_places, replacement=True):
        ...
        return await self.send_request(request_string, DECIMAL_METHOD)

    async def generate_gaussians(self, n, mean, standard_deviation,
                           significant_digits):
        ...
        return await self.send_request(request_string, GAUSSIAN_METHOD)

    async def generate_strings(self, n, length, characters, replacement=True):
        ...
        return await self.send_request(request_string, STRING_METHOD)

    async def generate_uuids(self, n):
        '''Returns a list of true random UUIDs (version 4)'''
        request_string = await compose_api_call(
            UUID_METHOD, apiKey=self.api_key, n=n)
        return await self.send_request(request_string, UUID_METHOD)

    async def generate_blobs(self, n, size, format=FORMAT_BASE64):
        '''Returns a list of Binary Large OBjects (BLOBs) containing
        true random data'''
        request_string = await compose_api_call(
            BLOB_METHOD, apiKey=self.api_key, n=n, size=size, format=format)
        return await self.send_request(request_string, BLOB_METHOD)
    ...
    async def get_usage(self):
        ...
        return await self.send_request(request_string, USAGE_METHOD)

####################### Digitally-signed API methods ##########################

    async def generate_signed_integers(self, n, min, max, replacement=True, base=10):
        request_string = await compose_api_call(
            SIGNED_INTEGER_METHOD, apiKey=self.api_key, n=n, min=min, max=max,
            replacement=replacement, base=base)
        return await self.send_request(request_string, SIGNED_INTEGER_METHOD)

    async def generate_signed_integer_sequences(self, n, length, min, max, replacement=True, base=10):
        request_string = await compose_api_call(
            SIGNED_INTEGER_SEQUENCE_METHOD, apiKey=self.api_key, n=n, length=length, min=min,
            max=max, replacement=replacement, base=base)
        return await self.send_request(request_string, SIGNED_INTEGER_SEQUENCE_METHOD)

    async def generate_signed_decimal_fractions(self, n, decimal_places,
                                          replacement=True):
        request_string = await compose_api_call(
            SIGNED_DECIMAL_METHOD, apiKey=self.api_key,
            n=n, decimalPlaces=decimal_places, replacement=replacement)
        return await self.send_request(request_string, SIGNED_DECIMAL_METHOD)

    async def generate_signed_gaussians(self, n, mean, standard_deviation,
                                  significant_digits):
        request_string = await compose_api_call(
            ...
            standardDeviation=standard_deviation,
            significantDigits=significant_digits)
        return await self.send_request(request_string, SIGNED_GAUSSIAN_METHOD)

    async def generate_signed_strings(self, n, length, characters, replacement=True):
        request_string = await compose_api_call(
            SIGNED_STRING_METHOD, apiKey=self.api_key,
            n=n, length=length, characters=characters, replacement=replacement)
        return await self.send_request(request_string, SIGNED_STRING_METHOD)

    async def generate_signed_uuids(self, n):
        request_string = await compose_api_call(
            SIGNED_UUID_METHOD, apiKey=self.api_key, n=n)
        return await self.send_request(request_string, SIGNED_UUID_METHOD)

    async def generate_signed_blobs(self, n, size, format=FORMAT_BASE64):
        request_string = await compose_api_call(
            SIGNED_BLOB_METHOD,
            apiKey=self.api_key, n=n, size=size, format=format)
        return await self.send_request(request_string, SIGNED_BLOB_METHOD)

    async def get_result(self, serial_number):
        ...
        response = await self.send_request(request_string, RESULT_METHOD)
        response._method = response._random['method']
        return response

    async def verify_signature(self, random, signature):
        """
        ...
        json_string = await compose_api_call(
            VERIFY_SIGNATURE_METHOD, random=self._random,
            signature=signature)

        response = await self.send_request(json_string, VERIFY_SIGNATURE_METHOD)
